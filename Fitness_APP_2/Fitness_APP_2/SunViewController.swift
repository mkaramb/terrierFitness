...
class SunViewController: UIViewController {
    ...
    let chestList = ["Push ups", "Bench press", "Dips", "Dumbbell press"].shuffled()
    var chestIndex = 0
    ...
    let legsList = ["Hack squats", "Leg extensions", "Calf raises", "Leg press", "Bar squats", "Deadlifts"].shuffled()
    var legsIndex = 0
    
    let coreList = ["Sit ups", "Russian twists", "Planks", "Hanging leg raises", "Tree cutters"].shuffled()
    var coreIndex = 0

    let cardioList = ["Running", "Cycling", "Elliptical", "Treadmill", "Jumping jacks", "Jump rope"].shuffled()
    var cardioIndex = 0
    ...
    @IBAction func tapAddButton(_ sender: Any) {
        if var txt = myInput.text, !txt.isEmpty {
            if setsRepsToggle.isOn {
                txt += " - " + setsLabel.text! + " sets, " + repsLabel.text! + " reps"
            }
